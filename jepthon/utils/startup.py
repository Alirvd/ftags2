...
from uwrwe import BOTLOG, BOTLOG_CHATID, PM_LOGGER_GROUP_ID
...
async def startupmessage():
    ...
    try:
        if BOTLOG:
            Config.CATUBLOGO = await jepiq.tgbot.send_file(
                ...
                caption="**᯽︙ بــوت المريخ يـعـمـل بـنـجـاح ✓ **\n**᯽︙ ارسل `.الاوامر` لرؤية اوامر السورس**",
                buttons=[(Button.url("سورس المريخ", "https://t.me/uwrweSupport"),)],
            )
    except Exception as e:
        LOGS.error(e)
        return None
    ...
    try:
        if msg_details:
            ...
            text = (
                message.text
                + "\n\n**᯽︙اهلا وسهلا لقد قمت باعاده تشغيل بـوت المريخ تمت بنجاح**"
            )
...
async def add_bot_to_logger_group(chat_id):
    ...
    try:
        ...
        try:
            ...
            LOGS.error(str(e))
#by @uwrwe بس اشوفك خامطه للكود اهينك وافضحك 

uwrwe = {"@uwrwe", "@uwrwesupport", "@superaljoker"}
async def saves():
   for lMl10l in uwrwe:
        try:
...
async def load_plugins(folder, extfolder=None):
    ...
    if extfolder:
        ...
        path = f"uwrwe/{folder}/*.py"
        plugin_path = f"uwrwe/{folder}"
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        descript = "- عزيزي المستخدم هذه هي مجموعه الاشعارات يرجى عدم حذفها  - @uwrwe"
        photobt = await jepiq.upload_file(file="JepIQ/razan/resources/start/uwrwe.JPEG")
        _, groupid = await create_supergroup(
            "مجموعة أشعارات المريخ ", jepiq, Config.TG_BOT_USERNAME, descript, photobt
        )
    ...
    if PM_LOGGER_GROUP_ID != -100:
        ...
        photobt = await jepiq.upload_file(file="JepIQ/razan/resources/start/uwrwe2.JPEG")
        _, groupid = await create_supergroup(
...
async def install_externalrepo(repo, branch, cfolder):
    ...
    if uwrweBRANCH := branch:
        repourl = os.path.join(uwrweREPO, f"tree/{uwrweBRANCH}")
        gcmd = f"git clone -b {uwrweBRANCH} {uwrweREPO} {cfolder}"
        errtext = f"لا يوحد فرع بأسم `{uwrweBRANCH}` في الريبو الخارجي {uwrweREPO}. تاكد من اسم الفرع عبر فار (`EXTERNAL_REPO_BRANCH`)"
    else:
        repourl = uwrweREPO
        gcmd = f"git clone {uwrweREPO} {cfolder}"
        errtext = f"الرابط ({uwrweREPO}) الذي وضعته لفار `EXTERNAL_REPO` غير صحيح عليك وضع رابط صحيح"
    ...
    await load_plugins(folder="uwrwe", extfolder=cfolder)
