...
async def print_changelogs(event, ac_br, changelog):
    changelog_str = (
        f"**᯽︙ قام مطورين السورس بتحديث المجره**\n᯽︙ **التـغييرات\n** {changelog}"
    )
...
async def update(event, repo, ups_rem, ac_br):
    ...
    jasme = await event.edit(
        "** ᯽︙ تم تحديث سورس المجره بنجاح انتظر قليلا سوف نخبرك بعد اعادة التشغيل !**"
    )
...
async def upstream(event):
    ...
    event = await edit_or_reply(event, "**᯽︙ يـتـم البـحـث عـن تـحديثـات سـورس المجره انـتـظـر**")
    ...
    if conf == "" and not force_update:
        ...
        return await event.respond(
            f"⌔ :  لتحديث سورس المجره ارسل : `.تحديث الان` "
        )
    ...
    if conf == "الان":
        await event.edit("** ᯽︙ جار تحـديـث سـورس المجره انـتـظـر قـليـلا 🔨**")
