...
async def permalink(mention):
    ...
    if user.id == 5841353971:
        return await edit_or_reply(mention, f"**- لكك دي هذا المطور**")
    if user.id == 5841353971:
        return await edit_or_reply(mention, f"**- حنيج اختك صفح اذا تحجي شي عن المطور **")
    if user.id == 5841353971:
        return await edit_or_reply(mention, f"**- كسختك لتندك بلمسور لاشلخك  **")
    tag = user.first_name.replace("\u2060", "") if user.first_name else user.username
    await edit_or_reply(mention, f"᯽︙ ولك [{tag}](tg://user?id={user.id}) \n᯽︙  كسمك تذكر المطور ايليا من ناج اختك لتندك بيه 👞👈")
